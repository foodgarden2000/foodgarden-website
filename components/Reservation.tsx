          ...
          <div className="bg-white/5 backdrop-blur-md rounded-xl shadow-2xl p-8 md:p-12 border border-white/10">
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="flex gap-4 mb-8">
                <button type="button" onClick={() => setFormData({...formData, type: 'table_booking'})} className={`flex-1 py-3 rounded-lg border font-bold text-[10px] uppercase tracking-widest transition-all ${formData.type === 'table_booking' ? 'border-brand-gold bg-brand-gold/10 text-brand-gold' : 'border-gray-700 text-gray-500'}`}>Table Booking</button>
                <button type="button" onClick={() => setFormData({...formData, type: 'cabin_booking'})} className={`flex-1 py-3 rounded-lg border font-bold text-[10px] uppercase tracking-widest transition-all ${formData.type === 'cabin_booking' ? 'border-brand-gold bg-brand-gold/10 text-brand-gold' : 'border-gray-700 text-gray-500'}`}>Private Cabin</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="group relative z-0 w-full mb-2">
                  <input type="text" name="name" required value={formData.name} onChange={handleChange} className="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-600 appearance-none focus:outline-none focus:ring-0 focus:border-brand-gold peer font-sans" placeholder=" " />
                ...
                <div className="group relative z-0 w-full mb-2">
                  <input type="tel" name="phone" required value={formData.phone} onChange={handleChange} className="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-600 appearance-none focus:outline-none focus:ring-0 focus:border-brand-gold peer font-sans" placeholder=" " />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="group relative z-0 w-full mb-2">
                  <input type="datetime-local" name="date" required value={formData.date} onChange={handleChange} className="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-600 appearance-none focus:outline-none focus:ring-0 focus:border-brand-gold peer font-sans scheme-dark" placeholder=" " />
                  <label className="peer-focus:font-medium absolute text-sm text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-brand-gold peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Booking Date & Time</label>
                </div>
                <div className="group relative z-0 w-full mb-2">
                  <select name="guests" value={formData.guests} onChange={handleChange} className="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-600 appearance-none focus:outline-none focus:ring-0 focus:border-brand-gold peer font-sans">
                    {[1, 2, 3, 4, 5, 6, 7, 8, '8+'].map(num => <option key={num} value={num} className="bg-brand-black">{num} People</option>)}
                  </select>
                  <label className="peer-focus:font-medium absolute text-sm text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-brand-gold peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Number of Guests</label>
                </div>
              </div>
              <div className="group relative z-0 w-full mb-2">
                <textarea name="message" value={formData.message} onChange={handleChange} className="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-600 appearance-none focus:outline-none focus:ring-0 focus:border-brand-gold peer font-sans h-24 resize-none" placeholder=" " />
                <label className="peer-focus:font-medium absolute text-sm text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-brand-gold peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Special Instructions</label>
              </div>
              <div className="text-center pt-4">
                <button type="submit" className="w-full py-4 bg-brand-gold text-brand-black font-bold text-sm uppercase tracking-widest hover:bg-white transition-all duration-300 rounded shadow-lg">Confirm Booking Request</button>
              </div>
            </form>
          </div>
        </div>

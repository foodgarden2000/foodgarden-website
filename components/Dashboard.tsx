...
import { 
  User, ShoppingBag, Clock, ShieldCheck, Zap, Ban, Package, Tag, Coins, Loader2
} from 'lucide-react';
import { 
  doc, 
  onSnapshot, 
...
const Dashboard: React.FC<DashboardProps> = ({ user, points, adminOnlyRequest, referralCodeFromUrl }) => {
  ...
  const getStatusDisplay = (status: OrderStatus) => {
    switch (status) {
      ...
      default: return { label: status.replace(/_/g, ' '), color: 'text-rose-500 bg-rose-500/10 border-rose-500/20' };
    }
  };
        ...
        {activeTab === 'profile' && (
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-fade-in">
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                 <h3 className="text-xl font-serif font-bold text-brand-black mb-6">Loyalty Balance</h3>
                 <div className="bg-brand-dark p-8 rounded-2xl flex items-center justify-between border-2 border-brand-gold shadow-lg">
                    <div className="w-16 h-16 bg-brand-gold/20 rounded-full flex items-center justify-center text-brand-gold"><Coins size={32} /></div>
                    <div className="text-right">
                       <p className="text-4xl font-display font-bold text-brand-gold">{profile?.points || 0}</p>
                       <p className="text-gray-400 text-[10px] uppercase tracking-widest">Available Jalsa Points</p>
                    </div>
                 </div>
              </div>
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
